...
class RegisterActivityViewModel(val authRepostory: AuthRepostory, val application: Application) :
    ViewModel() {
    ...
    fun validateEmailAddress(body: ValidateEmaiBody) {
        viewModelScope.launch {
            authRepostory.validateEmailAddress(body).collect {
                when (it) {
    ...
    fun registerUser(body: RegisterBody) {
        viewModelScope.launch {
            authRepostory.registerUser(body).collect {
                when (it) {
                    is RequestStatus.Wating -> {
                        isLocading.value = true
                    }
